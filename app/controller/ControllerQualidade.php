...
class ControllerQualidade extends ClassCrud
{
    ...
    public function PesquisarDadosBD()
    {
        ...
        while($Fetch=$BFetch->fetch(\PDO::FETCH_ASSOC)){
           ...
           $n = new ClassOption();
            $tb="
            <form name='LancAltQual' id='LancAltQual' action='".DIRPAGE."Home1/AlterarAb' method='post'>
            <td>".$n->InsertStatus($Status)."</td>";
            $tp="      
            <td style='visibility:hidden;'>
                <input type='hidden' name='Id[]' value='". $Id."'>
            </td>";
            $tt="<button id='BtnAlteQuali' class='btn btn-warning btn-xs'>
...
echo $tabela;
    ...
    private function recVariaveis()
    {
        if (isset($_POST['DataP'])) {
            $this->Data = filter_input(INPUT_POST, 'DataP', FILTER_SANITIZE_SPECIAL_CHARS);
        }
        if (isset($_POST['Pront'])) {
            $this->Pront = filter_input(INPUT_POST, 'Pront', FILTER_SANITIZE_SPECIAL_CHARS);
        }
        if (isset($_POST['Turno'])) {
            $this->Turno = filter_input(INPUT_POST, 'Turno', FILTER_SANITIZE_SPECIAL_CHARS);
        }
